...
bool Card::getpriceandfees(int & price, int & feesnumber, int cardnum,bool count, Grid* pGrid)
{/*its to read the parameters of the cards 9-11 to prevent the repeation of code*/
	if (cardnum >= 9 || cardnum <= 11)
	{
		...
		if (count)
		{
			string message = "please enter card " + to_string(cardnum)+"price";
			pOut->PrintMessage(message);
			
				 price = pIn->GetInteger(pOut);
				if (price <= 0)
				{
					pGrid->PrintErrorMessage("the enetered price is invalid ");
					price = 0;
					return false;
				}
				else
				{
					message = "please enter card " + to_string(cardnum) + " fees";
					pOut->PrintMessage(message);
					...
					if (feesnumber <= 0)
					{
						pGrid->PrintErrorMessage("the enetered price is invalid please enter a positive price");
						feesnumber = 0;
						return false;
					}
					else
...
void Card::apply9to11(Grid* pGrid, Player* pPlayer, Player* & Pplayer1, int cardprice,int fees)
{//it to apply the cards 9-11 to prevent the repeation of code
	Output* pOut = pGrid->GetOutput();
	Input* pIn = pGrid->GetInput();
