...
public:
	...
	bool getpriceandfees(int &price, int & feesnumber, int cardnum, bool count, Grid* pGrid);
	void SetCardNumber(int cnum);   // The setter of card number
	int GetCardNumber();            // The getter of card number
	void apply9to11(Grid* pGrid, Player* pPlayer, Player* & Pplayer1, int cardprice, int fees);
