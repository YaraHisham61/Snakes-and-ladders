...
public:
	...
	void SetPosition(CellPosition p);
	// ============ Virtual Functions ============
	...
	virtual void Save(ofstream &OutFile, int type, string file) = 0;	// Saves the GameObject parameters to the file
	virtual void Load(ifstream &Infile,int type,string file) = 0;	// Loads and Reads the GameObject parameters from the file
