...
bool Grid::overlapping(GameObject* pNewObject)
{
	...
	if (pos.IsValidCell())
	{
		...
		{
			if (CellList[pos.VCell()][i]->HasLadder() != NULL)
			{
				if (Ladder* newLadder = dynamic_cast<Ladder*>(pNewObject))
				{
					Ladder* ladder = CellList[pos.VCell()][i]->HasLadder();
					newLadder->overlappingladder(ladder);
					return true;
				}
			}
			if (CellList[pos.VCell()][i]->HasSnake() != NULL)
			{
				if (Snake* newSnake = dynamic_cast<Snake*>(pNewObject))
				{
					Snake* snake = CellList[pos.VCell()][i]->HasSnake();
					snake->GetPosition();
					newSnake->overlappingsnake(snake);
					return true;
				}
			}
		}
...
//void Grid::Setcardposition(CellPosition p)
//{
//	card
//}
