...
{
	int x = cellPos.HCell() * UI.CellWidth;///TODO: implement the following function as described in Output.h file
...
{
	int y = (cellPos.VCell() * UI.CellHeight) + UI.ToolBarHeight;///TODO: implement the following function as described in Output.h file
...
void Output::DrawCardNumber(const CellPosition & cellPos, int cardNum) const
{
	...
	pWind->GetIntegerSize(w, h, cardNum); ///TODO: Calculate the Width & Height of the integer "cardNum" if written using the Current Font
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_DELETE_GAME_OBJECT] = "images\\DeleteGameObject.jpg";
	MenuItemImages[ITM_OPEN_GRID] = "images\\OpenGrid.jpg";
	MenuItemImages[ITM_PASTE_CARD] = "images\\Pastecard.jpg";
	MenuItemImages[ITM_SAVE_GRID] = "images\\SaveGrid.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_NEW_GAME] = "images\\NewGame.jpg";
	MenuItemImages[ITM_INPUT_DICE_VALUE] = "images\\InputDiceValue.jpg";
	MenuItemImages[ITM_EXIT_PLAYMODE] = "images\\ExitPlaymode.jpg";
	///TODO: Prepare images for each menu item and add it to the list
...
{
	CreatePlayModeToolBar(); ///TODO: Clear what was written on the toolbar
	...
	pWind->GetStringSize(w, h, info); ///TODO: Calculate the Width and Height of the string if drawn using the current font 
...
void Output::DrawCell(const CellPosition & cellPos, int cardNum) const
{
	...
	pWind->GetIntegerSize(w, h, cellNum);///TODO: Get the Width and Height of the Cell Number if written using the current font 
	//       (Use GetIntegerSize() window function) and set the "w" and "h" variables with its width and height
	...
	pWind->DrawInteger(x, y, cellNum); ///TODO: Draw the cell number in the x and y location
...
void Output::DrawPlayer(const CellPosition & cellPos, int playerNum, color playerColor) const
{
	...
	if (playerNum < 0 || playerNum > 3) //second check
		return;///TODO: Validate the playerNum, if not valid return
	...
	pWind->DrawCircle(x, y, radius,FILLED); ///TODO: Draw the player circle in center(x,y) and filled with the playerColor passed to the function
...
void Output::DrawLadder(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	if (fromCell.HCell() != toCell.HCell() || toCell.VCell() > fromCell.VCell())
		return;
	...
	///TODO: Draw the cross horizontal lines of the ladder using the appropriate coordinates
        ...
        int y;
		for (int i = 0;i <abs (fromCell.VCell() - toCell.VCell());i++)
		{
			pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);
			y = fromStartY - i * UI.CellHeight;
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	//       Check the snakes drawn in the project document and draw it the same way
        int x[4] = { cellStartX + UI.SnakelineWidth / 4, cellStartX + xChange + UI.SnakelineWidth / 4,cellStartX + 2 * xChange + UI.SnakelineWidth / 4,cellStartX + xChange + UI.SnakelineWidth / 4 };
	int y[4] = { fromStartY + 2 * yChange,fromStartY + yChange,fromStartY + 2 * yChange,fromStartY + 3 * yChange };
